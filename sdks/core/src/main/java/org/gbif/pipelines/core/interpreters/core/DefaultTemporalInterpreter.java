...
public class DefaultTemporalInterpreter {
  ...
  public void interpretTemporal(ExtendedRecord er, TemporalRecord tr) {
    ...
    if (eventResult.isSuccessful()) {
      ...
      if(eventResult.getConfidence() == CONFIDENCE.POSSIBLE){
        tr.getIssues().getIssueList().add(RECORDED_DATE_AMBIGUOUS);
      }
   ...
   * @return interpretation result, never null
   */
  public  OccurrenceParseResult<TemporalAccessor> interpretRecordedDate(String year, String month, String day,
      String dateString) {
    ...
    ParseResult<TemporalAccessor> parsedYMDResult = atomizedDateProvided ? temporalParser
        .parse(year, month, day) :
        ParseResult.fail();
    ParseResult<TemporalAccessor> parsedDateResult = dateStringProvided ? temporalParser.parse(dateString) :
        ParseResult.fail();
   ...
   */
  public  OccurrenceParseResult<TemporalAccessor> interpretLocalDate(String dateString,
      Range<LocalDate> likelyRange,
      OccurrenceIssue unlikelyIssue) {
    if (!Strings.isNullOrEmpty(dateString)) {
      OccurrenceParseResult<TemporalAccessor> result = new OccurrenceParseResult<>(temporalParser.parse(dateString));
